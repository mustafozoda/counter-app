...
export interface ProductDetailViewProps {
  ...
  embedded?: boolean;
  /** Called after the product is deleted while embedded, to clear the selection. */
  onClose?: () => void;
}

/**
 * Product detail — the full record with stock, variants and history. Drives the
 * phone route (`product/[id]`) and the desktop master-detail pane; `embedded`
 * drops the screen chrome and back button so it sits inside the right pane.
 */
export function ProductDetailView({ id, embedded = false, onClose }: ProductDetailViewProps) {
  ...
  // Pick the outer container: full Screen on the phone route, a bare themed
  // surface when embedded in the desktop pane (the shell owns the safe area).
  ...
  const toggleArchive = () =>
    setStatus.mutate(
      { id: product.id, status: archived ? 'active' : 'archived' },
      {
        onSuccess: () =>
          toast.success(
            archived ? t('product.productRestored') : t('product.productArchived'),
            product.name,
          ),
      },
    );
    ...
    <>
      <View className="flex-row items-center justify-between px-5 pt-2">
        ...
        <View className="flex-row gap-2">
          ...
          <IconButton
            icon={Trash2}
            accessibilityLabel={t('product.deleteProduct')}
                  ...
                  >
                    <View className="flex-row items-center gap-3 px-4 py-3.5">
                      ...
                      <IconButton
                        ...
                        iconSize={16}
                        variant="surface"
                        accessibilityLabel={t('product.adjustStockFor', {
                ...
                return (
                  ...
                  >
                    <View
                      ...
                      style={{
                        backgroundColor: positive ? colors.positiveTint : colors.negativeTint,
                      }}
                    >
                      <meta.icon
                        size={16}
